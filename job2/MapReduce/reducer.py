...
def reducer():
    ...
    industry_per_year_data = defaultdict(lambda: defaultdict(lambda: [0, []]))

    for tokens in read_mapper_output(sys.stdin):

        sector, industry, year, date, ticker, close, volume = tokens
        close = float(close)
        volume = int(volume)
        year = int(year)

        # Aggiorna i dati per volume e prezzi di chiusura
        ticker_data = industry_per_year_data[(sector, industry, year)][ticker]
        ticker_data[0] += volume  # Accumula il volume
        ticker_data[1].append((date, close))  # Aggiungi data e prezzo di chiusura

    # Lista per memorizzare i risultati
    results = []

    # Ora dobbiamo trovare il ticker con il volume massimo e con il maggior incremento percentuale
    for (sector, industry, year), ticker_data in industry_per_year_data.items():
        max_volume = 0
        ...
        results.append((sector, year, industry, industry_percent_change, max_ticker_increase, max_increase, max_ticker_volume, max_volume))
    # Ordina i risultati per settore e variazione percentuale decrescente
    results.sort(key=lambda x: (x[0], -x[3]))

    # Stampa i risultati
    for sector, year, industry, industry_percent_change, max_ticker_increase, max_increase, max_ticker_volume, max_volume in results:        
        max_increase_output = f"{max_ticker_increase} ({max_increase:.2f})"
        max_volume_output = f"{max_ticker_volume} ({max_volume})"
        industry_price_change_output = f"{industry_percent_change:.2f}"
